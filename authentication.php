...
class Authentication
{
    ...
    public function signup($json)
    {
        ...
        try {
            if (isset($json['username']) && isset($json['password']) && isset($json['firstname']) && isset($json['lastname']) && isset($json['email'])) {
                $username = $json['username'];
                $password = sha1($json['password']);
                ...
                $sql = 'INSERT INTO `users` (`username`, `password`, `firstname`, `lastname`, `email`, `status`, `user_image`) 
...
$auth_endpoint = new Authentication();
